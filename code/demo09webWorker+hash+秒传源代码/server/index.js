...
// 获取hash是否重复确定文件是否上传过 上传过就不用前端再次上传 直接返回 完成秒传操作
router.get('/bigfile/existhash', (ctx) => {
  let { hash } = ctx.query;
  let hashList = fs.readdirSync(path.join(__dirname, '../static/uploads'));
  let resObj = {
    status: 200,
    hashList: hashList,
  };
  // console.log(hashList[0]), hash) // be70cfba5100638a3cbf40f8b78f0f7d.avi be70cfba5100638a3cbf40f8b78f0f7d
  if (hashList.length === 1 && hashList[0].startsWith(hash)) {
    ctx.body = JSON.stringify({
      status: 200,
      filesUrl: uploadHost + hashList[0],
    });
  } else {
...
router.post('/bigfile/upload', (ctx) => {
  let files = [];
  ...
  files &&
    files.forEach((file) => {
      ...
      if (filePath && file.size > 0) {
        ...
        let nextPath = filePath.slice(0, filePath.lastIndexOf('\\') + 1) + +fileIndex + '-' + fileHash;
  ...
  if (body.type === 'merge') {
    ...
    // console.log(folder)
    let writeStream = fs.createWriteStream(`${folder}${fileHash}${path.extname(filename)}`);
    let cindex = 0;
    mergeFileFn();
    ctx.body = JSON.stringify({
      status: 200,
      filesUrl: uploadHost + fileHash + path.extname(filename),
    });
    // 开始合并文件
    // 合并文件这里使用 stream pipe 实现，这样更节省内存，边读边写入，占用内存更小，效率更高，代码见fnMergeFile方法。
    function mergeFileFn() {
      let fname = `${folder}${cindex}-${fileHash}`;
